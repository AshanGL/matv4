...
-----------------------
FIX 1. _init_sandboxes: catches failures and puts None into the pool
        so the pool is never empty — prevents silent bail-out.
FIX 2. _attempt: uses get_nowait() and treats empty pool as sandbox=None
        (subprocess fallback) instead of returning None immediately.
FIX 3. _ATTEMPT_STRATEGY_HINTS[0]: now contains a strong computational
        strategy hint for large counting/DP problems.
"""
...
# FIX 3: attempt 0 now has a strong computational strategy hint
_ATTEMPT_STRATEGY_HINTS = [
    # attempt 0: strong push toward writing Python code for hard problems
    "\nSTRATEGY HINT: For counting/optimization problems over large ranges, "
    "write a complete Python solution using run_code. "
    "Think about the reachable state space after the first step to keep the DP small. "
    "Always print intermediate results. Use modular arithmetic only at the final step.",
...
class OlympiadSolver:
    ...
    # ── FIX 1: sandboxes are optional — None goes in pool on failure ──────────
    def _init_sandboxes(self):
        ...
        def _try_make_sandbox():
            try:
                ...
                print(f"  Sandbox init failed ({e}) — subprocess fallback will be used")
                return None
    ...
    def solve_problem(self, problem: str):
        ...
        # ── FIX 2: empty pool → sandbox=None (subprocess fallback), not bail-out
        def _attempt(idx):
            sandbox = None
            try:
                try:
                    ...
                    sandbox = None   # subprocess fallback — keep going
