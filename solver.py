...
2. Tokenizer loaded from encoder_name path, not ckpt_dir.
3. SYSTEM_PROMPT: type-aware strategy rules (not just format).
4. Fallback answer extraction: scans last 800 chars more aggressively.
5. Smarter "no answer" recovery with re-prompt at low temperature.
6. Per-attempt temperature variation for better diversity.
7. [NEW] Domain hint injected into user prompt via knowledge_search domain field.
8. [NEW] Type recovery prompt triggered when all answers are low-confidence.
9. [NEW] Attempt strategies vary more meaningfully (not just temperature):
   even attempts → symbolic-first, odd attempts → code-first.
10. [NEW] `problems_remaining` initialised from actual test set size.
11. [NEW] verify_scorer used in select_best_answer to re-rank candidates.
"""
...
from prompts import (
    ...
    build_retry_prompt, build_type_recovery_prompt, TOOL_CALL_FORMAT_REMINDER
)
...
class SolverConfig:
    ...
    # Generation
    max_tokens           = 16384
    temperature          = 0.6        # IMPROVEMENT 6: tighter base temperature
    min_p                = 0.02
    top_logprobs         = 5
    ...
    # Concurrency — IMPROVEMENT 9: 8 attempts for more votes
    attempts             = 8
    workers              = 8
    max_tool_turns       = 16         # IMPROVEMENT 9: more turns per attempt
    early_stop_votes     = 5          # IMPROVEMENT 9: higher threshold
    seed                 = 42
    batch_size           = 64
...
class AnswerTypeInference:
    ...
    - Load encoder from that path (which HAS model weights)
    - Load head from head_weights.pt (small, fast)
    - Tokenizer also loaded from encoder_name path
    """
...
def _fallback_extract(text: str, forced_type: Optional[str], problem: str,
    ...
    Scans the last 800 chars of assistant output.
    """
    tail = text[-800:]
...
# IMPROVEMENT 9: attempt strategy profiles
_ATTEMPT_STRATEGY_HINTS = [
    # attempt 0: default balanced
    "",
    # attempt 1: push symbolic computation first
    "\nSTRATEGY HINT: Prefer symbolic computation (compute tool) over code execution.",
    # attempt 2: push code/brute force first
    "\nSTRATEGY HINT: Use run_code for numerical verification before symbolic work.",
    # attempt 3: enumerate all cases explicitly
    "\nSTRATEGY HINT: Enumerate all cases explicitly. Don't skip edge cases.",
    # attempt 4: think about answer type carefully
    "\nSTRATEGY HINT: First determine the answer type (integer/expression/set/string) before computing.",
    # attempt 5: try a completely different approach
    "\nSTRATEGY HINT: Try a completely different solution approach from the standard method.",
    # attempt 6: check for multiple solutions
    "\nSTRATEGY HINT: Check whether the problem has MULTIPLE solutions and list all of them.",
    # attempt 7: verify numerically
    "\nSTRATEGY HINT: After getting an answer, use numerical_search to independently verify it.",
]
...
def run_attempt(
    ...
    domain:        Optional[str] = None,    # IMPROVEMENT 7
    ...
    # IMPROVEMENT 6+9: varied temperature AND strategy per attempt
    base_temp   = cfg.temperature
    temp_delta  = (attempt_idx - cfg.attempts // 2) * 0.05
    temperature = max(0.3, min(1.1, base_temp + temp_delta))

    seed = (cfg.seed + attempt_idx * 7) % (2**31)

    # IMPROVEMENT 9: strategy hint appended to system prompt for diversity
    strategy_hint = _ATTEMPT_STRATEGY_HINTS[attempt_idx % len(_ATTEMPT_STRATEGY_HINTS)]
    system_prompt = build_system_prompt(forced_type=forced_type) + strategy_hint
    ...
    try:
        ...
        # IMPROVEMENT 7: pass domain to build_user_prompt
        ...
        while turn < cfg.max_tool_turns:
            ...
            # IMPROVEMENT 8: type recovery if answer is low-confidence mid-solve
            if (typed is not None
                    and typed.confidence < 0.4
                    and not type_recovery_done
                    and turn >= 4):
                recovery = build_type_recovery_prompt(
                    problem, str(typed.value), typed.answer_type)
                messages.append({"role": "user", "content": recovery})
                type_recovery_done = True
...
class OlympiadSolver:
    ...
    # IMPROVEMENT 7: detect domain from knowledge search results
    def _detect_domain(self, problem: str) -> Optional[str]:
        try:
            from tools import _embed_query
            emb = _embed_query(problem)
            results = self.db.search_problems(emb, top_k=3)
            if results:
    ...
    def solve_problem(self, problem: str):
        ...
        # IMPROVEMENT 7: detect domain for richer prompts
        ...
        # IMPROVEMENT 11: re-score with verify_scorer if available
        if self.verify_scorer is not None:
            for ans in results:
                try:
                    _, score = self.verify_scorer.score(problem, str(ans.value))
                    # Blend verify_scorer score into confidence
                    ans.confidence = 0.7 * ans.confidence + 0.3 * score
                except Exception:
                    pass
